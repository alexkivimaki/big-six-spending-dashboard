...
"""Collect raw player market value history from a local Transfermarkt API service."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="data/raw/transfermarkt/market_values")
...
def main() -> int:
    ...
    endpoint = config["market_values_endpoint_template"].format(
        transfermarkt_player_id=args.player_id
    )
    url = f"{normalize_base_url(config['base_url'])}{endpoint}"
    collected_at = datetime.now(timezone.utc).isoformat()
    json_path = output_dir / f"{args.player_id}_market_values_raw.json"
    txt_path = output_dir / f"{args.player_id}_market_values_raw.txt"
    meta_path = output_dir / f"{args.player_id}_market_values_raw.meta.json"
